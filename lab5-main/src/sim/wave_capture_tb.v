...
module wave_capture_tb;
  ...
  initial begin
    ...
    // Record display buffer index at the moment we start capture (for later flip test)
    ...
    for (k = 1; k <= 255; k = k + 1) begin
      s = (k * 157) - 16'sd20000;
      pulse_and_check_write(s, 1'b1, {cap_write_half, (k & 8'hFF)}, exp_u8(s), "active write");
    end
  ...
  always @(posedge clk) begin
    if (write_enable)
      $display("WRITE: addr=%h count=%h", write_address, dut.count);
  end
